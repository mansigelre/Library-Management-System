...
const AddBookPopup = () => {
  ...
  dispatch(toggleAddBookPopup()); // <-- yahan popup band ho jayega
};
 useEffect(() => {
  if (message) {
    toast.success(message);
    dispatch(fetchAllBooks());
    // dispatch(toggleAddBookPopup()); // ab yahan zarurat nahi
    setTitle("");
    setAuthor("");
    setPrice("");
    setQuantity("");
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleAddBook}>
            ...
            <div className="mb-4">
              ...
              placeholder="Quantity" className="w-full px-4 py-2 border-2 border-black rounded-md" 
              required/>
            </div>

            <div className="mb-4">
              <label className="block text-gray-900 font-medium">Description</label>
              <textarea value={description} onChange={(e)=> setDescription(e.target.value)} placeholder="Book's Description" rows={4}
